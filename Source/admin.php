...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_user'])) {
    ...
    if ($step === 'register' && empty($_SESSION['user'])) { header("Location: admin.php?step=success"); }
...
$editUser = null;
if (isset($_GET['edit']) && isset($userData[$_GET['edit']])) { $editUser = $userData[$_GET['edit']]; $step = 'register'; }
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8"><title>Vlyx Administration</title>
...
<body>
    <div class="panel">
        ...
        <?php elseif ($step === 'register'): ?>
            ...
            <form method="POST">
                ...
                <?php if (!empty($userData)): ?>
                    <select name="role"><option value="user" <?= (isset($editUser['role']) && $editUser['role'] === 'user') ? 'selected' : '' ?>>Standard User</option><option value="admin" <?= (isset($editUser['role']) && $editUser['role'] === 'admin') ? 'selected' : '' ?>>Administrator</option></select>
                <?php endif; ?>
                <button type="submit" name="save_user" class="btn-vlyx"><?= $editUser ? 'Update' : 'Create' ?></button>
            </form>
        <?php elseif ($step === 'manage'): ?>
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;"><h2 style="margin:0;">Management</h2><a href="?step=register" class="badge">+ Add User</a></div>
            <table><?php foreach ($userData as $name => $info): ?><tr><td><strong><?= htmlspecialchars($name) ?></strong></td><td><span class="badge"><?= htmlspecialchars($info['role'] ?? 'user') ?></span></td><td style="text-align:right;"><a href="?edit=<?= urlencode($name) ?>" style="color:var(--accent); margin-right:15px;"><i class="fa-solid fa-user-pen"></i></a><?php if ($name !== $_SESSION['user']): ?><a href="?delete=<?= urlencode($name) ?>" style="color:#ff4444;" onclick="return confirm('Delete?')"><i class="fa-solid fa-trash-can"></i></a><?php endif; ?></td></tr><?php endforeach; ?></table>
        ...
        <?php elseif ($step === 'success'): ?>
            <h2>Setup Complete!</h2><p>System initialized. Please log in manually to verify your new credentials.</p><a href="login.php" class="btn-vlyx">Proceed to Login</a>
        <?php endif; ?>
    </div>
